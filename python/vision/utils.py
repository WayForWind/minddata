...
class ConvertMode(IntEnum):
    ...
    def to_c_type(mode):
        ...
        if mode is None:
            raise RuntimeError("Unsupported ConvertMode"
                               "dataset_vision/.dataset.vision.ConvertColor.html for more details.")
        return mode
...
def encode_jpeg(image, quality=75):
    ...
    Args:
        image (Union[numpy.ndarray, .Tensor]): The image to be encoded.
    ...
    Raises:
        TypeError: If `image` is not of type numpy.ndarray or .Tensor.
...
def read_image(filename, mode=ImageReadMode.UNCHANGED):
    ...
    Raises:
        TypeError: If `filename` is not of type str.
        TypeError: If `mode` is not of type :class:`.dataset.vision.ImageReadMode` .
    ...
    Examples:
        >>> import .dataset.vision as vision
        >>> from .dataset.vision import ImageReadMode
...
def write_file(filename, data):
    ...
    Args:
        ...
        data (Union[numpy.ndarray, .Tensor]): The one dimension uint8 data to be written.

    Raises:
        TypeError: If `filename` is not of type str.
        TypeError: If `data` is not of type numpy.ndarray or .Tensor.
...
def write_jpeg(filename, image, quality=75):
    ...
    Args:
        filename (str): The path to the file to be written.
        image (Union[numpy.ndarray, .Tensor]): The image data to be written.
