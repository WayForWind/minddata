...
from .dataset.core.validator_helpers import check_non_negative_float32, check_non_negative_int32, \
...
def create_dct(n_mfcc, n_mels, norm=NormMode.NONE):
    ...
    Raises:
        ...
        TypeError: If `n_mels` is not of type :class:`.dataset.audio.NormMode` .

    Supported Platforms:
        ``CPU``

    Examples:
        >>> from .dataset.audio import create_dct, NormMode
        >>>
...
def linear_fbanks(n_freqs, f_min, f_max, n_filter, sample_rate):
    ...
    Examples:
        >>> from .dataset.audio import linear_fbanks
        >>>
...
def melscale_fbanks(n_freqs, f_min, f_max, n_mels, sample_rate, norm=NormType.NONE, mel_type=MelType.HTK):
    ...
    Examples:
        >>> from .dataset.audio import melscale_fbanks
        >>>
