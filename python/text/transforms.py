...
class AddToken(TextTensorOperation):
    ...
    Tutorial Examples:
        - `Illustration of text transforms
          <https://www..cn/docs/en/master/api_python/samples/dataset/text_gallery.html>`_
    """
...
class JiebaTokenizer(TextTensorOperation):
    ...
    Examples:
        >>> import .dataset as ds
        >>> import .dataset.text as text
        >>> from .dataset.text import JiebaMode
        >>>
        >>> # Use the transform in dataset pipeline mode
...
class Lookup(TextTensorOperation):
    ...
    Args:
        ...
        data_type (.dtype, optional): The data type that lookup operation maps
    ...
    Raises:
        ...
        TypeError: If `data_type` is not of type .dtype.
...
class Ngram(TextTensorOperation):
    ...
    Args:
        ...
        separator (str, optional): Symbol used to join strings together.
...
class SentencePieceTokenizer(TextTensorOperation):
    ...
    Examples:
        >>> import .dataset as ds
        >>> import .dataset.text as text
        >>> from .dataset.text import SentencePieceModel, SPieceTokenizerOutType
        >>>
        >>> # Use the transform in dataset pipeline mode
        >>> numpy_slices_dataset = ds.NumpySlicesDataset(data=['Hello world'], column_names=["text"])
        >>>
...
class ToNumber(TextTensorOperation):
    ...
    Args:
        data_type (.dtype): Type to be cast to. Must be a numeric type in .dtype.
...
class WordpieceTokenizer(TextTensorOperation):
    ...
    Raises:
        TypeError: If `vocab` is not of type :class:`.dataset.text.Vocab` .
...
if platform.system().lower() != 'windows':
    ...
    class BasicTokenizer(TextTensorOperation):
        ...
        Examples:
            >>> import .dataset as ds
            >>> import .dataset.text as text
            >>> from .dataset.text import NormalizeForm
            >>>
            >>> # Use the transform in dataset pipeline mode
            >>> numpy_slices_dataset = ds.NumpySlicesDataset(data=['Welcome     To   BeiJing!'], column_names=["text"])
            >>>
