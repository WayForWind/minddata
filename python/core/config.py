...
from .dataset.core.validator_helpers import replace_none, type_check, check_valid_str
from .dataset.debug import DebugHook, PrintMetaDataHook
from .dataset.core.validator_helpers import check_independent_mode
...
def _init_device_info():
    ...
    from .parallel._utils import _get_global_rank
    numa_enable = False
...
def set_seed(seed):
    ...
    Note:
        ...
        and `.dataset` modules to the specified value at the same time.
...
def set_auto_num_workers(enable):
    ...
    :func:`.dataset.config.set_num_parallel_workers`.
...
def set_callback_timeout(timeout):
    """
    Set the default timeout (in seconds) for :class:`.dataset.WaitedDSCallback` .

    Args:
        timeout (int): Timeout (in seconds) to be used to end the wait in :class:`.dataset.WaitedDSCallback`
...
def get_callback_timeout():
    """
    Get the default timeout (in seconds) for :class:`.dataset.WaitedDSCallback` .
    If `set_callback_timeout` is never called before, the default value(60) will be returned.

    Returns:
        int, Timeout (in seconds) to be used to end the wait in :class:`.dataset.WaitedDSCallback` in case of
        a deadlock.
...
def set_enable_autotune(enable, filepath_prefix=None):
    ...
    Note:
        - Saved parameter profiles can be loaded via the :func:`.dataset.deserialize` interface to
...
def set_sending_batches(batch_num):
    ...
    be enabled via the :class:`.train.Model.train` interface.
...
def set_debug_mode(debug_mode_flag: bool, debug_hook_list: list = None):
    ...
    Examples:
        ...
        >>>
        >>> # 2. Enable dataset pipeline debug mode and use pre-defined debug hook provided by OURS.
