    ...
    def add_child(self, sampler):
        ...
        Args:
            sampler (Sampler): Object used to choose samples from the dataset. Only builtin
                samplers(:class:`.dataset.DistributedSampler` ,
                :class:`.dataset.PKSampler`,
                :class:`.dataset.RandomSampler`,
                :class:`.dataset.SequentialSampler`,
                :class:`.dataset.SubsetRandomSampler`,
                :class:`.dataset.WeightedRandomSampler` ) are supported.
...
class DistributedSampler(BuiltinSampler):
    ...
    Args:
        ...
        shuffle (Union[bool, Shuffle], optional): Specify the shuffle mode.
            Default: ``True``, performs ``.dataset.Shuffle.GLOBAL`` . If `shuffle` is ``False`` ,
            no shuffling will be performed.
            There are several levels of shuffling, desired shuffle enum defined by :class:`.dataset.Shuffle` .
        ...
        offset(int, optional): The starting shard ID where the elements in the dataset are sent to, which
            ...
            a :class:`.dataset.DistributedSampler` as its sampler. It will affect the number of
...
class PKSampler(BuiltinSampler):
    ...
    Args:
        ...
        class_column (str, optional): Name of column with class labels for Dataset. Default: ``'label'``.
        num_samples (int, optional): The number of samples to draw. Default: ``None`` , which means sample all elements.
