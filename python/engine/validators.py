...
def check_generator_dataset(method):
    ...
    def new_method(self, *args, **kwargs):
        ...
        check_list = ['.nn', '.ops', '.numpy', '.compression']
        ...
        if sampler is not None:
            ...
            if not isinstance(sampler, samplers.BuiltinSampler):
                try:
                    ...
                    raise TypeError("sampler should be either iterable or from .dataset.samplers.")
...
def get_map_kwargs_from_dict(param_dict):
    ...
    return python_multiprocessing, max_rowsize,  callbacks, offload
...
def check_map(method):
    ...
    @wraps(method)
    def new_method(self, *args, **kwargs):
        from .dataset.callback import DSCallback
        ...
        (python_multiprocessing, max_rowsize, callbacks, offload) = get_map_kwargs_from_dict(param_dict)
        ...
        for item in operations:
            if isinstance(item, (nn.Cell, ops.Primitive)):
                raise ValueError("Input operations should not contain network computing operator like in "
                                 ".nn or .ops, got operation: ", str(item))
