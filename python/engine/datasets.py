...
from parallel._ps_context import _is_role_pserver, _is_role_sched, _get_ps_context, \
...
from dataset.text.utils import SentencePieceModel, DE_C_INTER_SENTENCEPIECE_MODE
from dataset.debug import DebugHook

from dataset.engine import samplers
from dataset.engine.samplers import Shuffle
...
class Dataset:
        ...
        def process_name(datasets, operator_id):
            ...
            for item in datasets:
                for d in item.children:
                    ...
                    from dataset.engine.datasets_user_defined import GeneratorDataset
        ...
        Args:
            ...
            **kwargs:
                ...
                - max_rowsize(Union[int, list[int]], optional): Maximum size of row in MB that is used for shared memory
                  ...
                  ``num_parallel_workers`` and :func:`dataset.config.set_prefetch_size` increase.
    ...
    def map(self, operations, input_columns=None, output_columns=None, column_order=None,
        ...
        - If you use transformations (
          `vision transform <https://cn/docs/en/master/api_python/\
          dataset.transforms.html#module-dataset.vision>`_ ,
          `nlp transform <https://cn/docs/en/master/api_python/\
          dataset.transforms.html#module-dataset.text>`_ ,
          `audio transform <https://cn/docs/en/master/api_python/\
          dataset.transforms.html#module-dataset.audio>`_ )
          provided by OURS dataset, please use the following parameters:
        ...
        Note:
            - Input `operations` accepts TensorOperations defined in dataset part, plus user-defined
            ...
            - Setting the start method of multiprocessing to `spawn` mode by
              ...
              and `num_parallel_workers>1` supports adding network computing operators from nn and
              ops or other network computing operators into this `operations` .
        ...
        if hasattr(self, 'operator_mixed') and getattr(self, 'operator_mixed') is True:
            ...
            logger.warning(
                "Input 'operations' of 'map' includes network computing operators like in nn, ops, "
                "numpy module and etc, which do not support multithreading compiling, recommend to replace "
    ...
    @check_save
    def save(self, file_name, num_files=1):
        """
...
class VisionBaseDataset(Dataset):
    ...
    def __init__(self, children=None, num_parallel_workers=None):
        super().__init__(children=children, num_parallel_workers=num_parallel_workers)
...
class TextBaseDataset(Dataset):
    ...
    def build_vocab(self, columns, freq_range, top_k, special_tokens, special_first):
        ...
        Note:
            dataset.Dataset.build_vocab is deprecated from version 2.0
            and will be removed in a future version. Use dataset.text.Vocab.from_dataset instead.
        ...
        """
        warnings.warn("dataset.Dataset.build_vocab is deprecated from version 2.0 "
                      "and will be removed in a future version. "
                      "Use dataset.text.Vocab.from_dataset instead.", DeprecationWarning)
    ...
    def build_sentencepiece_vocab(self, columns, vocab_size, character_coverage, model_type, params):
        ...
        Note:
            dataset.Dataset.build_sentencepiece_vocab is deprecated from version 2.0
            and will be removed in a future version. Use dataset.text.SentencePieceVocab.from_dataset instead.
        ...
        """
        warnings.warn("dataset.Dataset.build_sentencepiece_vocab is deprecated from version 2.0 "
                      "and will be removed in a future version. "
                      "Use dataset.text.SentencePieceVocab.from_dataset instead.", DeprecationWarning)
    ...
    def split(self, sizes, randomize=True):
        ...
        Note:
            ...
            2. Dataset should not be sharded if split is going to be called. Instead, create a
               :class:`dataset.DistributedSampler` and specify a split to shard after splitting.
...
class BatchDataset(UnionBaseDataset):
    ...
    Args:
        ...
        max_rowsize(Union[int, list[int]], optional): Maximum size of row in MB that is used for shared memory
            allocation to copy data between processes, the total occupied shared memory will increase as
            ``num_parallel_workers`` and :func:`dataset.config.set_prefetch_size` increase. If set to -1,
...
class MapDataset(UnionBaseDataset):
    ...
    def __init__(self, input_dataset, operations=None, input_columns=None, output_columns=None,
                 num_parallel_workers=None, python_multiprocessing=False, callbacks=None, max_rowsize=None):
        super().__init__(children=input_dataset, num_parallel_workers=num_parallel_workers)
...
class Schema:
    ...
    Examples:
        >>> import dataset as ds
        >>> from OURS import dtype as mstype
        >>>
        >>> # Create schema; specify column name, dtype and shape of the column
        >>> schema = ds.Schema()
