...
Status Iterator::GetNextRowCharIF(OURTensorMapChar *row) {
  ...
  for (auto &de_tensor : md_map) {
    ...
    row->insert(std::make_pair(col_name, ours::OURTensor(std::make_shared<DETensor>(de_tensor.second))));
  }
...
// Get the next row from the data pipeline.
Status Iterator::GetNextRow(OURTensorVec *row) {
  RETURN_UNEXPECTED_IF_NULL(row);
  // Clean data row
  row->clear();
  // create a dataset tensor row and fetch. Then we convert the output to OURTensor
  ...
  std::transform(md_row.begin(), md_row.end(), std::back_inserter(*row),
                 [](auto t) { return ours::OURTensor(std::make_shared<DETensor>(t)); });
  return Status::OK();
}
...
// Get the next row from the data pipeline.
Status PullIterator::GetRows(int32_t num_rows, std::vector<OURTensorVec> *const row) {
  RETURN_UNEXPECTED_IF_NULL(row);
  ...
  for (int i = 0; i < num_rows; i++) {
    ...
    for (const auto &de_tensor : md_row) {
      ...
      ms_row.push_back(ours::OURTensor(std::make_shared<DETensor>(de_tensor)));
    }
...
Status PullIterator::GetNextRow(OURTensorVec *const row) {
  RETURN_UNEXPECTED_IF_NULL(row);
  ...
  for (const auto &de_tensor : md_row) {
    CHECK_FAIL_RETURN_UNEXPECTED(de_tensor->HasData(), "Apply transform failed, output tensor has no data");
    row->push_back(ours::OURTensor(std::make_shared<DETensor>(de_tensor)));
  }
  return Status::OK();
}
