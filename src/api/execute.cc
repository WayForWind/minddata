...
#include "OURSdata/dataset/core/ascend_resource.h"
...
Status Execute::operator()(const ours::OURTensor &input, ours::OURTensor *output) {
  ...
  if (device_type_ == MapTargetDevice::kCpu) {
    // Convert ours::Tensor to dataset::Tensor
    ...
    // Convert dataset::Tensor to ours::Tensor
    ...
    *output = ours::OURTensor(std::make_shared<DETensor>(de_tensor));
    ...
    std::shared_ptr<ours::dataset::DeviceTensor> device_input;
    ...
    *output = ours::OURTensor(std::make_shared<DETensor>(device_input, true));
#endif
  } else {
...
Status Execute::operator()(const std::vector<OURTensor> &input_tensor_list, std::vector<OURTensor> *output_tensor_list) {
  // Validate input tensor
  ...
  if (device_type_ == MapTargetDevice::kCpu) {  // Case CPU
    ...
    for (auto &tensor : de_tensor_list) {
      ...
      auto ms_tensor = ours::OURTensor(std::make_shared<DETensor>(tensor));
    ...
    for (auto &input_tensor : input_tensor_list) {
      ...
      std::shared_ptr<ours::dataset::Tensor> host_output;
      RETURN_IF_NOT_OK(device_resource_->Pop(device_input, &host_output));

      auto ms_tensor = ours::OURTensor(std::make_shared<DETensor>(host_output));
      (void)output_tensor_list->emplace_back(ms_tensor);
...
Status Execute::Run(const std::vector<std::shared_ptr<dataset::Execute>> &data_graph,
                    const std::vector<ours::OURTensor> &inputs, std::vector<ours::OURTensor> *outputs) {
  RETURN_UNEXPECTED_IF_NULL(outputs);
  std::vector<OURTensor> transform_inputs = inputs;
  std::vector<OURTensor> transform_outputs;
  if (!data_graph.empty()) {
...
// In the current stage, there is a cyclic dependency between libours.so and c_dataengine.so,
...
extern "C" {
// ExecuteRun_C has C-linkage specified, but returns user-defined type 'ours::Status' which is incompatible with C
