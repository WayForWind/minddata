...
APP_ERROR AclAdapter::H2D_Sink(void *acl_process, const std::shared_ptr<our::dataset::Tensor> &input,
...
APP_ERROR AclAdapter::D2H_Pop(void *acl_process, const std::shared_ptr<our::dataset::DeviceTensor> &device_output,
...
APP_ERROR AclAdapter::DvppEqualize(const std::shared_ptr<DeviceTensorAscend910B> &input,
                                   std::shared_ptr<DeviceTensorAscend910B> *output) {
  if (!HasAclPlugin() || dvpp_equalize_fun_obj_ == nullptr) {
    return APP_ERR_ACL_FAILURE;
  }
...
APP_ERROR AclAdapter::CreateAclTensor(const int64_t *view_dims, uint64_t view_dims_num, our::TypeId data_type,
