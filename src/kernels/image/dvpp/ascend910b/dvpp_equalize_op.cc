...
Status DvppEqualizeOp::Compute(const std::shared_ptr<DeviceTensorAscend910B> &input,
  ...
  CHECK_FAIL_RETURN_UNEXPECTED(
    ...
    "DvppEqualize: the input tensor is not HW, HWC or 1HWC, but got: " + std::to_string(input->GetShape().Rank()));

  // Dvpp Limit
  int64_t input_h = input->GetShape()[kHeightIndexNHWC];
  int64_t input_w = input->GetShape()[kWidthIndexNHWC];
  RETURN_IF_NOT_OK(CheckDvppLimit(input_h, input_w, h_lb, w_lb, h_ub, w_ub, kDvppEqualizeOp));

  // run dvpp
  APP_ERROR ret = AclAdapter::GetInstance().DvppEqualize(input, output);
  if (ret != APP_ERR_OK) {
    std::string error = "DvppEqualize: Error in dvpp processing: " + std::to_string(ret);
