...
Status DvppSolarizeOp::Compute(const std::shared_ptr<DeviceTensorAscend910B> &input,
                               std::shared_ptr<DeviceTensorAscend910B> *output) {
  IO_CHECK(input, output);
  // check the input tensor shape
  if (input->GetShape().Rank() != kNHWCImageRank) {
    RETURN_STATUS_UNEXPECTED("DvppSolarize: invalid input shape, only support NHWC input, got rank: " +
  ...
  RETURN_IF_NOT_OK(CheckDvppLimit(input_h, input_w, h_lb, w_lb, h_ub, w_ub, kDvppAdjustHueOp));

  APP_ERROR ret = AclAdapter::GetInstance().DvppSolarize(input, output, threshold_);
  if (ret != APP_ERR_OK) {
    std::string error = "DvppSolarize: Error in dvpp processing: " + std::to_string(ret);
...
Status DvppSolarizeOp::OutputShape(const std::vector<TensorShape> &inputs, std::vector<TensorShape> &outputs) {
  RETURN_IF_NOT_OK(TensorOp::OutputShape(inputs, outputs));
  return Status::OK();
}

Status DvppSolarizeOp::OutputType(const std::vector<DataType> &inputs, std::vector<DataType> &outputs) {
