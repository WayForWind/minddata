 ...
 */
APP_ERROR MDAclProcess::H2D_Sink(const std::shared_ptr<our::dataset::Tensor> &input,
  ...
  const our::dataset::DataType dvpp_data_type(our::dataset::DataType::DE_UINT8);
  const our::dataset::TensorShape dvpp_shape({1, 1, 1});
  auto rc = our::dataset::DeviceTensor::CreateEmpty(dvpp_shape, dvpp_data_type, &device_input);
  if (rc.IsError()) {
...
APP_ERROR MDAclProcess::D2H_Pop(const std::shared_ptr<our::dataset::DeviceTensor> &device_output,
  ...
  our::dataset::dsize_t dvppDataSize = device_output->DeviceDataSize();
  const our::dataset::TensorShape dvpp_shape({dvppDataSize, 1, 1});
  ...
  const our::dataset::DataType dvpp_data_type(our::dataset::DataType::DE_UINT8);
  auto rc = our::dataset::Tensor::CreateFromMemory(dvpp_shape, dvpp_data_type, ret_ptr, &output);
  if (rc.IsError()) {
    MS_LOG(ERROR) << "Failed to allocate memory, error msg is " << rc;
    return APP_ERR_ACL_BAD_ALLOC;
  }
