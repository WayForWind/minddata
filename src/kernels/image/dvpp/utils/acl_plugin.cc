...
#include "OURSdata/dataset/kernels/image/dvpp/utils/acl_plugin.h"

#include <string>

#include "OURSdata/dataset/kernels/image/dvpp/utils/dvpp_video.h"
#include "OURSdata/dataset/kernels/image/dvpp/utils/ResourceManager.h"
#include "OURSdata/dataset/kernels/image/dvpp/utils/MDAclProcess.h"
#if defined(ENABLE_D)
...
APP_ERROR PluginH2D_Sink(void *acl_process, const std::shared_ptr<our::dataset::Tensor> &input,
                         std::shared_ptr<our::dataset::DeviceTensor> &device_input) {
  if (acl_process == nullptr) {
    return APP_ERR_ACL_FAILURE;
  }
...
APP_ERROR PluginD2H_Pop(void *acl_process, const std::shared_ptr<our::dataset::DeviceTensor> &device_output,
                        std::shared_ptr<our::dataset::Tensor> &output) {
  if (acl_process == nullptr) {
    return APP_ERR_ACL_FAILURE;
  }
...
#if defined(ENABLE_D)
// Ascend910B
APP_ERROR PluginDvppAdjustBrightness(const std::shared_ptr<our::dataset::DeviceTensorAscend910B> &input,
                                     std::shared_ptr<our::dataset::DeviceTensorAscend910B> *output,
                                     float factor) {
  if (input == nullptr) {
    return APP_ERR_ACL_FAILURE;
  }
...
APP_ERROR PluginDvppAdjustContrast(const std::shared_ptr<our::dataset::DeviceTensorAscend910B> &input,
                                   std::shared_ptr<our::dataset::DeviceTensorAscend910B> *output, float factor) {
  if (input == nullptr) {
    return APP_ERR_ACL_FAILURE;
  }
  if (output == nullptr) {
...
APP_ERROR PluginDvppAdjustHue(const std::shared_ptr<our::dataset::DeviceTensorAscend910B> &input,
                              std::shared_ptr<our::dataset::DeviceTensorAscend910B> *output, float factor) {
  if (input == nullptr) {
    return APP_ERR_ACL_FAILURE;
  }
...
APP_ERROR PluginDvppAdjustSaturation(const std::shared_ptr<our::dataset::DeviceTensorAscend910B> &input,
                                     std::shared_ptr<our::dataset::DeviceTensorAscend910B> *output,
                                     float factor) {
  if (input == nullptr) {
    return APP_ERR_ACL_FAILURE;
...
APP_ERROR PluginDvppAdjustSharpness(const std::shared_ptr<our::dataset::DeviceTensorAscend910B> &input,
                                    std::shared_ptr<our::dataset::DeviceTensorAscend910B> *output, float factor) {
  if (input == nullptr) {
    return APP_ERR_ACL_FAILURE;
  }
...
APP_ERROR PluginDvppAffine(const std::shared_ptr<our::dataset::DeviceTensorAscend910B> &input,
                           std::shared_ptr<our::dataset::DeviceTensorAscend910B> *output,
...
APP_ERROR PluginDvppAutoContrast(const std::shared_ptr<our::dataset::DeviceTensorAscend910B> &input,
                                 std::shared_ptr<our::dataset::DeviceTensorAscend910B> *output,
...
APP_ERROR PluginDvppConvertColor(const std::shared_ptr<our::dataset::DeviceTensorAscend910B> &input,
                                 std::shared_ptr<our::dataset::DeviceTensorAscend910B> *output,
                                 our::dataset::ConvertMode convertMode) {
  if (input == nullptr) {
    return APP_ERR_ACL_FAILURE;
  }
  if (output == nullptr) {
...
APP_ERROR PluginDvppCrop(const std::shared_ptr<our::dataset::DeviceTensorAscend910B> &input,
                         std::shared_ptr<our::dataset::DeviceTensorAscend910B> *output, uint32_t top,
...
APP_ERROR PluginDvppDecode(const std::shared_ptr<our::dataset::DeviceTensorAscend910B> &input,
                           std::shared_ptr<our::dataset::DeviceTensorAscend910B> *output) {
  if (input == nullptr) {
    return APP_ERR_ACL_FAILURE;
  }
...
APP_ERROR PluginDvppEqualize(const std::shared_ptr<our::dataset::DeviceTensorAscend910B> &input,
                             std::shared_ptr<our::dataset::DeviceTensorAscend910B> *output) {
  if (input == nullptr) {
    return APP_ERR_ACL_FAILURE;
  }
...
APP_ERROR PluginDvppErase(const std::shared_ptr<our::dataset::DeviceTensorAscend910B> &input,
                          std::shared_ptr<our::dataset::DeviceTensorAscend910B> *output, uint32_t top,
...
APP_ERROR PluginDvppGaussianBlur(const std::shared_ptr<our::dataset::DeviceTensorAscend910B> &input,
                                 std::shared_ptr<our::dataset::DeviceTensorAscend910B> *output,
...
APP_ERROR PluginDvppHorizontalFlip(const std::shared_ptr<our::dataset::DeviceTensorAscend910B> &input,
                                   std::shared_ptr<our::dataset::DeviceTensorAscend910B> *output) {
  if (input == nullptr) {
    return APP_ERR_ACL_FAILURE;
  }
...
APP_ERROR PluginDvppInvert(const std::shared_ptr<our::dataset::DeviceTensorAscend910B> &input,
                           std::shared_ptr<our::dataset::DeviceTensorAscend910B> *output) {
  if (input == nullptr) {
    return APP_ERR_ACL_FAILURE;
  }
...
APP_ERROR PluginDvppNormalize(const std::shared_ptr<our::dataset::DeviceTensorAscend910B> &input,
                              std::shared_ptr<our::dataset::DeviceTensorAscend910B> *output,
...
APP_ERROR PluginDvppPad(const std::shared_ptr<our::dataset::DeviceTensorAscend910B> &input,
                        std::shared_ptr<our::dataset::DeviceTensorAscend910B> *output,
...
APP_ERROR PluginDvppPerspective(const std::shared_ptr<our::dataset::DeviceTensorAscend910B> &input,
                                std::shared_ptr<our::dataset::DeviceTensorAscend910B> *output,
                                const std::vector<std::vector<int32_t>> &start_points,
                                const std::vector<std::vector<int32_t>> &end_points,
                                our::dataset::InterpolationMode interpolation) {
  if (input == nullptr) {
    return APP_ERR_ACL_FAILURE;
  }
  if (output == nullptr) {
...
APP_ERROR PluginDvppPosterize(const std::shared_ptr<our::dataset::DeviceTensorAscend910B> &input,
                              std::shared_ptr<our::dataset::DeviceTensorAscend910B> *output, uint8_t bits) {
  if (input == nullptr) {
    return APP_ERR_ACL_FAILURE;
  }
  if (output == nullptr) {
...
APP_ERROR PluginDvppResize(const std::shared_ptr<our::dataset::DeviceTensorAscend910B> &input,
                           std::shared_ptr<our::dataset::DeviceTensorAscend910B> *output, int32_t output_height,
                           int32_t output_width, double fx, double fy, our::dataset::InterpolationMode mode) {
  if (input == nullptr) {
    return APP_ERR_ACL_FAILURE;
  }
  if (output == nullptr) {
...
APP_ERROR PluginDvppResizedCrop(const std::shared_ptr<our::dataset::DeviceTensorAscend910B> &input,
                                std::shared_ptr<our::dataset::DeviceTensorAscend910B> *output, int32_t top,
                                int32_t left, int32_t height, int32_t width, int32_t output_height,
                                int32_t output_width, our::dataset::InterpolationMode mode) {
  if (input == nullptr) {
    return APP_ERR_ACL_FAILURE;
  }
  if (output == nullptr) {
...
APP_ERROR PluginDvppRotate(const std::shared_ptr<our::dataset::DeviceTensorAscend910B> &input,
                           std::shared_ptr<our::dataset::DeviceTensorAscend910B> *output, float degrees,
                           our::dataset::InterpolationMode mode, bool expand, const std::vector<float> &center,
                           const std::vector<float> &fill) {
  if (input == nullptr) {
    return APP_ERR_ACL_FAILURE;
  }
...
APP_ERROR PluginDvppSolarize(const std::shared_ptr<our::dataset::DeviceTensorAscend910B> &input,
                             std::shared_ptr<our::dataset::DeviceTensorAscend910B> *output,
...
APP_ERROR PluginDvppVerticalFlip(const std::shared_ptr<our::dataset::DeviceTensorAscend910B> &input,
                                 std::shared_ptr<our::dataset::DeviceTensorAscend910B> *output) {
  if (input == nullptr) {
    return APP_ERR_ACL_FAILURE;
  }
...
// acl
APP_ERROR PluginGetSocName(std::string *soc_name) { return our::dataset::GetSocName(soc_name); }

APP_ERROR PluginCreateAclTensor(const int64_t *view_dims, uint64_t view_dims_num, our::TypeId data_type,
  ...
  return our::dataset::CreateAclTensor(view_dims, view_dims_num, data_type, stride, offset, storage_dims,
...
APP_ERROR PluginDestroyTensor(void *tensor) {
  ...
  return our::dataset::DestroyTensor(tensor);
}
...
APP_ERROR PluginDestroyFloatArray(void *float_array) {
  ...
  return our::dataset::DestroyFloatArray(float_array);
}
...
APP_ERROR PluginDestroyIntArray(void *int_array) {
  ...
  return our::dataset::DestroyIntArray(int_array);
}
#endif
