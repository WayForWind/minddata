...
namespace ours {
namespace dataset {
YahooAnswersNode::YahooAnswersNode(const std::string &dataset_dir, const std::string &usage, int64_t num_samples,
...
void YahooAnswersNode::Print(std::ostream &out) const {
  out << (Name() + "(cache: " + ((cache_ != nullptr) ? "true" : "false") +
          ", num_shards: " + std::to_string(num_shards_) + ", shard_id: " + std::to_string(shard_id_) + ")");
}

Status YahooAnswersNode::ValidateParams() {
  ...
  RETURN_IF_NOT_OK(ValidateDatasetShardParams("YahooAnswersNode", num_shards_, shard_id_));

  return Status::OK();
}

// Function to build YahooAnswersNode
Status YahooAnswersNode::Build(std::vector<std::shared_ptr<DatasetOp>> *const node_ops) {
  ...
  for (auto c : column_names) {
    column_default_list.push_back(std::make_shared<YahooAnswersOp::Record<std::string>>(YahooAnswersOp::STRING, ""));
  }

  std::shared_ptr<YahooAnswersOp> yahoo_answers_op = std::make_shared<YahooAnswersOp>(
  ...
  RETURN_IF_NOT_OK(yahoo_answers_op->Init());

  // If a global shuffle is used for YahooAnswers, it will inject a shuffle op over the YahooAnswers.
  // But, if there is a cache in the tree, we do not need the global shuffle and the shuffle op should not be built.
  // This is achieved in the cache transform pass where we call MakeSimpleProducer to reset YahooAnswers's shuffle
  // option to false.
  if (shuffle_ == ShuffleMode::kGlobal) {
    ...
    RETURN_IF_NOT_OK(YahooAnswersOp::CountAllFileRows(sorted_dataset_files, column_names.empty(), &num_rows));
  ...
  yahoo_answers_op->SetTotalRepeats(GetTotalRepeats());
  yahoo_answers_op->SetNumRepeatsPerEpoch(GetNumRepeatsPerEpoch());
  node_ops->push_back(yahoo_answers_op);

  return Status::OK();
}

Status YahooAnswersNode::WalkAllFiles(const std::string &dataset_dir, const std::string &usage,
...
Status YahooAnswersNode::GetShardId(int32_t *shard_id) {
  *shard_id = shard_id_;
...
// Get Dataset size
Status YahooAnswersNode::GetDatasetSize(const std::shared_ptr<DatasetSizeGetter> &size_getter, bool estimate,
  ...
  RETURN_IF_NOT_OK(YahooAnswersOp::CountAllFileRows(dataset_files, column_names.empty(), &num_rows));
...
// YahooAnswers by itself is a non-mappable dataset that does not support sampling.
// However, if a cache operator is injected at some other place higher in the tree, that cache can
// inherit this sampler from the leaf, providing sampling support from the caching layer.
// That is why we setup the sampler for a leaf node that does not use sampling.
Status YahooAnswersNode::SetupSamplerForCache(std::shared_ptr<SamplerObj> *sampler) {
  *sampler = SelectSampler(num_samples_, shuffle_, num_shards_, shard_id_);
  return Status::OK();
}

// If a cache has been added into the ascendant tree over this YahooAnswers node, then the cache will be executing
// a sampler for fetching the data.  As such, any options in the YahooAnswers node need to be reset to its defaults so
// that this YahooAnswers node will produce the full set of data into the cache.
Status YahooAnswersNode::MakeSimpleProducer() {
  shard_id_ = 0;
  num_shards_ = 1;
