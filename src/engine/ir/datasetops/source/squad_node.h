...
#ifndef OURS_CCSRC_OURSdata_DATASET_ENGINE_IR_DATASETOPS_SOURCE_SQUAD_NODE_H_
#define OURS_CCSRC_OURSdata_DATASET_ENGINE_IR_DATASETOPS_SOURCE_SQUAD_NODE_H_
...
namespace dataset {
/// \class SQuADNode.
/// \brief A Dataset derived class to represent SQuAD dataset.
class SQuADNode : public NonMappableSourceNode {
 public:
  /// \brief Constructor.
  SQuADNode(const std::string &dataset_dir, const std::string &usage, int64_t num_samples, ShuffleMode shuffle,
  ...
  std::string Name() const override { return kSQuADNode; }
  ...
  /// \brief SQuAD by itself is a non-mappable dataset that does not support sampling.
  ...
  /// \brief If a cache has been added into the ascendant tree over this SQuAD node, then the cache will be executing
...
#endif  // OURS_CCSRC_OURSdata_DATASET_ENGINE_IR_DATASETOPS_SOURCE_SQUAD_NODE_H_
