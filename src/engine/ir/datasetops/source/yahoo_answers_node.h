...
#ifndef OURS_CCSRC_OURSdata_DATASET_ENGINE_IR_DATASETOPS_SOURCE_YAHOO_ANSWERS_NODE_H
#define OURS_CCSRC_OURSdata_DATASET_ENGINE_IR_DATASETOPS_SOURCE_YAHOO_ANSWERS_NODE_H
...
namespace ours {
namespace dataset {
class YahooAnswersNode : public NonMappableSourceNode {
 public:
  /// \brief Constructor.
  YahooAnswersNode(const std::string &dataset_dir, const std::string &usage, int64_t num_samples, ShuffleMode shuffle,
                   int32_t num_shards, int32_t shard_id, std::shared_ptr<DatasetCache> cache);

  /// \brief Destructor.
  ~YahooAnswersNode() override = default;

  /// \brief Node name getter.
  /// \return Name of the current node.
  std::string Name() const override { return kYahooAnswersNode; }
  ...
  /// \brief YahooAnswers by itself is a non-mappable dataset that does not support sampling.
  ...
  /// \brief If a cache has been added into the ascendant tree over this YahooAnswers node, then the cache will be
  ///     executing a sampler for fetching the data. As such, any options in the YahooAnswers node need to be reset
  ///     to its defaults so that this YahooAnswers node will produce the full set of data into the cache. Note:
...
#endif  // OURS_CCSRC_OURSdata_DATASET_ENGINE_IR_DATASETOPS_SOURCE_YAHOO_ANSWERS_NODE_H_
