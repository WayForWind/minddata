...
// In the current stage, there is a cyclic dependency between libour.so and c_dataengine.so,
...
extern "C" {
// ParseMindIRPreprocess_C has C-linkage specified, but returns user-defined type 'our::Status'
// which is incompatible with C
void ParseMindIRPreprocess_C(const std::vector<std::string> &dataset_json,
                             std::vector<std::shared_ptr<our::dataset::Execute>> *data_graph, Status *s) {
  if (s != nullptr) {
