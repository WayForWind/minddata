...
namespace ours {
namespace dataset {
AmazonReviewOp::AmazonReviewOp(int32_t num_workers, int64_t num_samples, int32_t worker_connector_size,
                               ...
                               const std::vector<std::string> &amazon_review_files_list)
    : CsvOp(amazon_review_files_list, field_delim, column_default, column_name, num_workers, num_samples,
            worker_connector_size, op_connector_size, shuffle_files, num_devices, device_id) {}

void AmazonReviewOp::Print(std::ostream &out, bool show_all) const {
  if (!show_all) {
