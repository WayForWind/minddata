...
#ifndef OURS_CCSRC_OURSdata_DATASET_CORE_TENSOR_H_
#define OURS_CCSRC_OURSdata_DATASET_CORE_TENSOR_H_
...
class DATASET_API Tensor {
 public:
  ...
  /// Create a copy of the input tensor
  /// \param[in] in OURTensor to create DETensor from.
  /// \param[in] out DETensor created.
  /// \return Status
  static Status CreateFromOURTensor(const OURTensor &in, TensorPtr *out);

#ifdef ENABLE_PYTHON
  /// Create a Tensor from a given py::array and reuse the memory of numpy
  ...
  Status Slice(TensorPtr *out, const std::vector<our::dataset::SliceOption> &slice_options);
...
#endif  // OURS_CCSRC_OURSdata_DATASET_CORE_TENSOR_H_
