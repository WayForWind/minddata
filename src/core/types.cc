...
class TensorDefaultImpl : public OURTensor::Impl {
 public:
...
class TensorReferenceImpl : public OURTensor::Impl {
 public:
  TensorReferenceImpl()
...
OURTensor *OURTensor::CreateTensor(const std::vector<char> &name, enum DataType type, const std::vector<int64_t> &shape,
  ...
  try {
    ...
    OURTensor *ret = new OURTensor(impl);
    return ret;
...
OURTensor *OURTensor::CreateTensor(const std::vector<char> &name, const OURTensor &tensor, const std::vector<char> &device,
                                 int device_id) noexcept {
  MS_LOG(ERROR) << "Invalid implement.";
  return nullptr;
}

OURTensor *OURTensor::CreateRefTensor(const std::vector<char> &name, enum DataType type,
...
OURTensor OURTensor::CreateDeviceTensor(const std::vector<char> &name, enum DataType type,
  ...
  try {
    ...
    return OURTensor(impl);
  } catch (const std::bad_alloc &) {
    MS_LOG(ERROR) << "Malloc memory failed.";
    return OURTensor(nullptr);
  } catch (...) {
    MS_LOG(ERROR) << "Unknown error occurred.";
    return OURTensor(nullptr);
  }
...
OURTensor *OURTensor::CreateTensorFromFile(const std::vector<char> &file, enum DataType type,
  ...
  try {
    ...
    OURTensor *ret = new OURTensor(file_path, type, tensor_shape, nullptr, size);
...
OURTensor *OURTensor::CharStringsToTensor(const std::vector<char> &name, const std::vector<std::vector<char>> &str) {
...
std::vector<std::vector<char>> OURTensor::TensorToStringChars(const OURTensor &tensor) {
...
void OURTensor::DestroyTensorPtr(OURTensor *tensor) noexcept {
  if (tensor != nullptr) {
    delete tensor;
  }
}

OURTensor::OURTensor() : impl_(std::make_shared<TensorDefaultImpl>()) {}
OURTensor::OURTensor(std::nullptr_t) : impl_(nullptr) {}
OURTensor::OURTensor(const std::shared_ptr<Impl> &impl) : impl_(impl) { MS_EXCEPTION_IF_NULL(impl); }
OURTensor::OURTensor(const std::vector<char> &name, enum DataType type, const std::vector<int64_t> &shape,
                   const void *data, size_t data_len)
    : impl_(std::make_shared<TensorDefaultImpl>(CharToString(name), type, shape, data, data_len)) {}
OURTensor::~OURTensor() = default;

bool OURTensor::operator==(std::nullptr_t) const { return impl_ == nullptr; }

bool OURTensor::operator!=(std::nullptr_t) const { return impl_ != nullptr; }

bool OURTensor::operator==(const OURTensor &tensor) const { return impl_ == tensor.impl_; }

bool OURTensor::operator!=(const OURTensor &tensor) const { return impl_ != tensor.impl_; }

OURTensor *OURTensor::Clone() const {
  MS_EXCEPTION_IF_NULL(impl_);
  try {
    OURTensor *ret = new OURTensor();
...
std::vector<char> OURTensor::CharName() const {
  MS_EXCEPTION_IF_NULL(impl_);
  return StringToChar(impl_->Name());
}

enum DataType OURTensor::DataType() const {
  MS_EXCEPTION_IF_NULL(impl_);
  return impl_->DataType();
}

const std::vector<int64_t> &OURTensor::Shape() const {
  MS_EXCEPTION_IF_NULL(impl_);
  return impl_->Shape();
}

int64_t OURTensor::ElementNum() const {
  MS_EXCEPTION_IF_NULL(impl_);
...
std::shared_ptr<const void> OURTensor::Data() const {
  MS_EXCEPTION_IF_NULL(impl_);
  return impl_->Data();
}

void *OURTensor::MutableData() {
  MS_EXCEPTION_IF_NULL(impl_);
  return impl_->MutableData();
}

size_t OURTensor::DataSize() const {
  MS_EXCEPTION_IF_NULL(impl_);
  return impl_->DataSize();
}

bool OURTensor::IsDevice() const {
  MS_EXCEPTION_IF_NULL(impl_);
  return impl_->IsDevice();
}

void OURTensor::SetShape(const std::vector<int64_t> &) { MS_LOG_EXCEPTION << "Invalid implement."; }

void OURTensor::SetDataType(enum DataType) { MS_LOG_EXCEPTION << "Invalid implement."; }

void OURTensor::SetTensorName(const std::vector<char> &) { MS_LOG_EXCEPTION << "Invalid implement."; }

void OURTensor::SetAllocator(std::shared_ptr<Allocator>) { MS_LOG_EXCEPTION << "Invalid implement."; }

std::shared_ptr<Allocator> OURTensor::allocator() const { MS_LOG_EXCEPTION << "Invalid implement."; }

void OURTensor::SetFormat(our::Format) { MS_LOG_EXCEPTION << "Invalid implement."; }

our::Format OURTensor::format() const { MS_LOG_EXCEPTION << "Invalid implement."; }

void OURTensor::SetData(void *, bool) { MS_LOG_EXCEPTION << "Invalid implement."; }

void OURTensor::SetDeviceData(void *) { MS_LOG_EXCEPTION << "Invalid implement."; }

void *OURTensor::GetDeviceData() { MS_LOG_EXCEPTION << "Invalid implement."; }

std::vector<QuantParam> OURTensor::QuantParams() const { MS_LOG_EXCEPTION << "Invalid implement."; }

void OURTensor::SetQuantParams(std::vector<QuantParam>) { MS_LOG_EXCEPTION << "Invalid implement."; }
