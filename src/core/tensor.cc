...
Status Tensor::CreateFromFile(const std::string &path, std::shared_ptr<Tensor> *out) {
  ...
  if (num_bytes >= kDeMaxDim) {
    fs.close();
    RETURN_STATUS_UNEXPECTED("Invalid file to allocate tensor memory, check path: " + path);
  }
  if (!fs.seekg(0, std::ios::beg).good()) {
    fs.close();
    RETURN_STATUS_UNEXPECTED("Failed to find size of file, check path: " + path);
  }
  auto s = Tensor::CreateEmpty(TensorShape{num_bytes}, DataType(DataType::DE_UINT8), out);
  if (s != Status::OK()) {
    fs.close();
    return s;
  }
...
Status Tensor::GetSliceOption(const SliceOption &slice_option, const int32_t &slice_index,
  ...
  if (slice_option.indices_.empty()) {
    ...
    our::dataset::Slice slice_copy = slice_option.slice_;
...
Status Tensor::Slice(std::shared_ptr<Tensor> *out, const std::vector<SliceOption> &slice_options) {
  ...
  for (size_t k = 0; k < slice_options.size(); k++) {
    ...
    if (slice_option.all_) {
      auto slice = our::dataset::Slice(shape_[static_cast<dsize_t>(k)]);
  ...
  if (slice_options.size() != Rank()) {
    for (auto j = static_cast<dsize_t>(slice_options.size()); j < Rank(); j++) {
      our::dataset::Slice slice = our::dataset::Slice(0, shape_[j]);
...
Status Tensor::CreateFromOURTensor(const OURTensor &in, TensorPtr *out) {
  if (in.Data() == nullptr) {
    *out = nullptr;
    return Status::OK();
  }
