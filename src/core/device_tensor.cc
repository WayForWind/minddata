...
Status DeviceTensor::DataPop_(std::shared_ptr<Tensor> *host_tensor) {
  ...
  our::dataset::dsize_t dvppDataSize = this->DeviceDataSize();
  ...
  RETURN_IF_NOT_OK(our::dataset::Tensor::CreateFromMemory(dvpp_shape, dvpp_data_type, ret_ptr, host_tensor));
